...
<script>
	...
	// 수정
	$("#ulboardUpdate").click(function() {
		if (confirm('수정 하시겠습니까?')) {
		document.form1.action = "${path}/ulboard/ulboardUpdate?idx=${view.idx}";
		document.firm1.submit();
		/* location.href = "${path}/ulboard/ulboardUpdate.do?idx=${view.idx}"; */
		}
	});
	// 삭제
	$("#ulboardDelete").click(function() {
		if (confirm('삭제 하시겠습니까?')) {
		document.form1.action = "${path}/ulboard/ulboardDelete.do?idx=${view.idx}";
		document.form1.submit();
		/* location.href = "${path}/ulboard/ulboardDelete.do?idx=${view.idx}"; */
		}
	});
	// 목록
	$("#ulboardList").click(function() {
...
<body>
	...
	<form method="post" name="form1" class="form-horizontal" enctype="multipart/form-data">
		<div class="alert alert-warning" role="alert">
			...
			<div align="center">
					<h4><span class="label label-info">제목</span>${view.title}</h4>
			</div>
			<hr />
			<div class="form-group">
				<label for="comment"></label> ${view.content}
			</div>
			<hr />
			<div class="alert">
				...
				<c:if test="${view.email==member.email}">
					...
					<ul class="nav navbar-nav navbar-right">
						<li><button id="ulboardList" class="btn btn-primary btn-group-sm">목록</button></li>
						<li>&nbsp;</li>
					</ul>
			</div>
		</div>
